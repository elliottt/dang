...
newtype LL a = LL
  { unLL :: ReaderT (Set.Set AST.Var) (StateT Subst (WriterT [Decl] Dang)) a
...
extend :: [(AST.Var,Term)] -> LL ()
extend ns = do
  u <- get
  set $! Map.union (Map.fromList ns) u

subst :: [AST.Var] -> AST.Var -> LL Term
subst args v = do
  u <- get
...
bindVars :: Set.Set AST.Var -> LL a -> LL a
...
-- function.
rewriteFreeVars :: [AST.Var] -> [AST.Decl] -> LL ()
